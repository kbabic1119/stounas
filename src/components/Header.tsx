...
export default function Header({ currentLang, setLang, bgIndex, setBgIndex, totalBackgrounds }: HeaderProps) {
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          {/* Right utility items: Language switcher, BG toggle, and Phone Call */}
          <div className="hidden md:flex items-center space-x-4">
            {/* Background Switcher */}
            <button
              onClick={() => setBgIndex((bgIndex + 1) % totalBackgrounds)}
              className="flex items-center space-x-1 bg-brand-dark-lighter border border-white/5 px-2.5 py-1.5 rounded-full text-xs font-bold font-mono text-zinc-400 hover:text-white hover:border-brand-primary/30 transition-all duration-200 cursor-pointer"
              title="Keisti foną"
            >
              <span>{bgIndex + 1}/{totalBackgrounds}</span>
      ...
      <div className={`md:hidden fixed inset-x-0 top-[73px] bg-brand-dark border-b border-white/10 shadow-2xl transition-all duration-300 transform ${
        ...
        <div className="px-4 pt-2 pb-6 space-y-1">
          ...
          <div className="pt-4 px-4 border-t border-white/5 flex flex-col space-y-4">
            ...
            <button
              onClick={() => setBgIndex((bgIndex + 1) % totalBackgrounds)}
              className="flex items-center justify-center space-x-2 w-full bg-brand-dark-lighter border border-white/5 py-3 rounded-xl text-sm font-bold font-mono text-zinc-400 hover:text-white hover:border-brand-primary/30 transition-all duration-200 cursor-pointer"
              title="Keisti foną"
            >
              <span>{bgIndex + 1}/{totalBackgrounds}</span>
